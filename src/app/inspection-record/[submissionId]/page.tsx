...
export default function InspectionRecordPage() {
  ...
  return <main className="mx-auto max-w-4xl px-6 py-8"><div className="no-print mb-5 flex justify-between"><Link href="/inspect" className="text-sm text-primary hover:underline">Back to inspections</Link><Button onClick={() => window.print()}><Printer className="h-4 w-4" /> Print</Button></div><article className="rounded-lg border border-border bg-white p-8 shadow-soft print:border-0 print:shadow-none"><div className="flex flex-col justify-between gap-5 border-b border-border pb-6 sm:flex-row sm:items-start"><div><h1 className="text-3xl font-semibold text-slate-950">Incoming Inspection Record</h1><p className="mt-2 text-sm text-muted-foreground">{submission.formName}</p></div><div className="rounded-md bg-accent px-4 py-3 text-right"><div className="text-xs font-semibold uppercase tracking-wide text-accent-foreground">Inspection Result</div><div className="mt-1 text-2xl font-semibold text-accent-foreground">{String(result ?? "Submitted")}</div></div></div><section className="mt-6 grid gap-4 sm:grid-cols-2"><Info label="Submission Timestamp" value={formatDateTime(submission.submittedAt)} /><Info label="Collection Plan" value={submission.collectionPlanName} /><Info label="Receipt Number" value={submission.receipt.receiptNumber} /><Info label="PO Number" value={submission.receipt.poNumber} /><Info label="Supplier" value={submission.receipt.supplier} /><Info label="Item" value={submission.receipt.item} /><Info label="Item Description" value={submission.receipt.itemDescription} /><Info label="Quantity Pending Inspection" value={`${submission.receipt.quantityPendingInspection} ${submission.receipt.uom}`} /></section><section className="mt-8"><h2 className="border-b border-border pb-2 text-lg font-semibold">Inspection Values</h2><div className="mt-4 grid gap-3" style={{ gridTemplateColumns: `repeat(${form?.layout?.columns ?? defaultPaperLayout.columns}, minmax(140px, 1fr))`, gridAutoRows: `minmax(${form?.layout?.rowHeightPx ?? defaultPaperLayout.rowHeightPx}px, auto)` }}>{form ? form.blocks?.map((block) => <RecordBlock key={block.id} block={block} values={submission.values} columns={form.layout?.columns ?? defaultPaperLayout.columns} />) : Object.entries(submission.values).map(([key, value]) => <Info key={key} label={key} value={value} />)}</div></section></article></main>;
}
